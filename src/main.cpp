// DearJack Audio DSP System
// This program creates a dynamic audio processing system using JACK and Dear ImGui.
// Multiple DSP (Digital Signal Processing) modules can be created, managed, and controlled via a GUI.
//
// Key Features:
// - Support for different DSP types 
// - Dynamic addition and removal of JACK clients, each with its own DSP module
// - GUI controls for modifying DSP parameters in real-time
//
// Components:
// - DSP base class: Defines the interface for all DSP modules.
// - DSP implementations: SinOsc, SquareWave, SawWave etc
// - JackClient class: Manages JACK client connections and processes audio through DSP modules.
// - DSPFactory class: Dynamically creates DSP instances based on type.
// - ImGui GUI: Provides interactive controls for managing JACK clients and modifying DSP parameters.
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    if (ImGui::Button("Add JackClient")) {
      ...
      jack_clients.push_back(std::make_unique<JackClient>(
          client_name.c_str(),
          DSPFactory::instance().create_dsp(selected_dsp_type)));
    }
